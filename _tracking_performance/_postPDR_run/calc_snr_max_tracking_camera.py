...
# old
def plot_results_byfield(teff,ao_mode,datapath='./output/h2rg_newPDR_byField/',filt_band='H'):
	"""
	...
	ao_mode = 'LGS_STRAP_130'
	teff=3600

	fig, ax = plt.subplots(1,1,figsize=(6,5),sharex=True,sharey=True)
	for jj,x in enumerate(field_r):
		framerates =np.zeros(len(track_bands))
		for ii,track_band in enumerate(track_bands):
			savename = 'cendata_%s_ao_%s_band_%s_teff_%s_%smag_fieldr_%sarcsec.npy' %(camera,ao_mode,track_band,teff,'H',field_r[jj])
		...
		ax.semilogy(track_bands,framerates,c=colors2[jj],linestyle=linestyles[jj],label='(%s")'%(field_r[jj]),lw=0.8,alpha=1)
	...
	plt.savefig('./output/frame_rate_fieldpositions_ao_mode_%s_Teff_%sK.png'%(ao_mode,teff))

############## FINAL
def plot_results_var_final(var,teff,datapath,camera,field_r,track_bands=['y','z','JHgap','J','H','K']):
	"""
	...
	for itexp in np.arange(len(exptimes)):
		for ii,track_band in enumerate(track_bands):
			for jj,ao_mode in enumerate(modes):
				savename = 'cendata_%s_ao_%s_band_%s_teff_%s_%smag_fieldr_%sarcsec.npy' %(camera,ao_mode,track_band,teff,'H',field_r)
				try: centroid = np.load(datapath + 'centroid_%s'%(savename))
				except: continue
...
def plot_results_mag_req_single(track_band='JHgap',filt_band='H',fieldr=0):
	"""
	"""
	...
	for ii,teff in enumerate([1500,2300,3600,5800]):
		for jj,ao_mode in enumerate(modes):
			savename = 'cendata_%s_ao_%s_band_%s_teff_%s_%smag_fieldr_%sarcsec.npy' %('h2rg',ao_mode,track_band,teff,'H',field_r)
			try: centroid = np.load(datapath + 'centroid_%s'%(savename))
			except: continue
			try:	mag_req = get_mag_limit_per_exptime(exptimes,magarr,centroid,tracking_requirement_pixel)
			except: mag_req = np.zeros_like(exptimes)
...
def plot_results_mag_req_final(teff,camera,filt_band='H',track_bands=['y','z','JHgap','J','H','K'],datapath='./output/trackingcamera/centroid_arrs/_run_20230313/'):
	...
	savename = 'mag_req_ao_mode_%s_Teff_%sK_%s.png'%(ao_mode,teff,camera)
	savepath = os.path.join(*datapath.split('/')[0:-1]) + '/_plots/'
	if not os.path.isdir(savepath): os.makedirs(savepath)
...
def plot_frame_rates_magnitude(teffs,ao_mode,camera,field_r,track_bands=['J','Jplus','H','JHgap'],maglimits=[11.9,12.9,13.9,14.9,15.9],filt_band='H',datapath = './output/trackingcamera/centroid_arrs/_run_20230313/'):
	"""
	must edit savename and link to folder in centroid/data/ 
	...
	for track_band in track_bands:
		...
		for ii,maglimit in enumerate(maglimits):
			if '%s' in ao_mode:
				if track_band=='H' or track_band=='K': ao_mode_filled = ao_mode%'H'
				else: ao_mode_filled = ao_mode%'J'
			else: ao_mode_filled=ao_mode
			savename = 'cendata_%s_ao_%s_band_%s_teff_%s_%smag_fieldr_%sarcsec.npy' %(camera,ao_mode_filled,track_band,teff,'H',field_r)
			try: centroid = np.load(datapath + 'centroid_%s'%(savename))
			except: continue
			try:	mag_req = get_mag_limit_per_exptime(exptimes,magarr,centroid,tracking_requirement_pixel)
			except: mag_req = np.zeros_like(exptimes)
	...
	savename = 'frame_rate_ao_mode_%s_Teff_%sK_%s.png'%(ao_mode,teff,camera)
	savepath = os.path.join(*datapath.split('/')[0:-1]) + '/_plots/'
	if not os.path.isdir(savepath): os.makedirs(savepath)
...
def run_all(so,path,track_bands):
	...
	magarr   = np.arange(7,18,1) # in band of interest
	...
	run_dic = {}
	#run_dic[1000] = ['LGS_100%s_130']
	#run_dic[1500] = ['LGS_100%s_130']
	#run_dic[2300] = ['LGS_100%s_130']
	#run_dic[3000] = ['LGS_100%s_130']
	run_dic[3600] = ['LGS_STRAP_130']
	#run_dic[4200] = ['LGS_STRAP_130']
	#run_dic[5800] = ['LGS_STRAP_130']
	teffs = run_dic.keys()
		...
		for hh,ao_mode in enumerate(run_dic[teff]):
			for jj, track_band in enumerate(track_bands):
				if '%s' in ao_mode: # fill in with J or H, or dont do it
					...
					elif track_band=='yJH' or track_band=='JHplus': ao_mode%'H'
...
def run_cameras():
	"""
	mostly storage bc hoping to not have to run multiple cameras again
	"""
	for i,camera in enumerate(['h2rg']):#,'cred2']):
		so.track.camera = camera
		so.track.fratio = [35, 30][i] # set f ratio for fair comp
		cload      = fill_data(so)

		# run
		#datapath = './output/trackingcamera/centroid_arrs/_run_20230313/'
		datestr  = date.today().strftime("%Y%m%d")#'20230406' #date.today().strftime("%Y%m%d")
		datapath = './output/_run_%s_%s/'%(datestr,camera)
	...
	# plot
	for i,camera in enumerate(['h2rg']):#,'cred2']):
		configfile = './hispec_tracking_camera.cfg' #datapath + 'hispec_tracking_camera.cfg'
		so         = load_object(configfile)
		so.track.camera = camera
		so.track.fratio = [35,30][i] # set f ratio for fair comp
		cload      = fill_data(so)
		#datapath   = './output/_run_LGSbrightTTstar_20230502_%s/'%camera
		maglimits = [11.9,12.9,13.9,14.9,15.9]
		datapath = './output/_run_%s_%s/'%(datestr,camera)
		for teff in [1000,1500,2300,3000,3600,4200,5800]:
			print("TEFF: %s"%teff)
			...
			#plot_results_mag_req_final(teff,camera,filt_band='H',track_bands=track_bands,datapath=datapath)
			track_bands = ['z','y','J','JHgap','H','K']
			plot_frame_rates_magnitude(teff,ao_mode,maglimits=maglimits,filt_band='H',camera=camera,track_bands=track_bands,field_r=so.track.field_r,datapath=datapath)
...
def run_off_axis():
	...
	for i,camera in enumerate(['h2rg']):#,'cred2']):
		...
		#datapath = './output/trackingcamera/centroid_arrs/_run_20230313/'
		datestr  = date.today().strftime("%Y%m%d")#'20230406' #date.today().strftime("%Y%m%d")
		datapath = './output/_run_fieldr_%s_%s_%s/'%(so.track.field_r,datestr,camera)
		track_bands = ['z','y','J','JHgap','H','K']
		run_all(so,datapath,track_bands=track_bands)
		os.system('cp %s %s' %(configfile, datapath)) # copy config file used to data path for record
	...
	for i,camera in enumerate(['h2rg']):#,'cred2']):
		...
		for teff in [1000,1500,2300,3000,3600,4200,5800]:
			...
			#plot_results_mag_req_final(teff,camera,filt_band='H',track_bands=track_bands,datapath=datapath)
			track_bands = ['z','y','J','JHgap','H','K']
			plot_frame_rates_magnitude(teff,ao_mode,field_r=so.track.field_r,maglimits=maglimits,filt_band='H',camera=camera,track_bands=track_bands,datapath=datapath)
